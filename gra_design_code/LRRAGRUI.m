...
global class_num;
global sample_num;
LRRAGR_class_num=int32(str2double(class_num));%样本种类数量
...
lambda1=str2double(get(handles.LRRAGR_UI_lambda1,'String'));%lambda1值
lambda2=str2double(get(handles.LRRAGR_UI_lambda2,'String'));%lambda2值
lambda3=str2double(get(handles.LRRAGR_UI_lambda3,'String'));%lambda3值
...
disp('绘制亲和矩阵...');
axes(handles.LRRAGR_UI_rep);%使用图像，操作在坐标2
L=get_affinity(Z,LRRAGR_class_num);
imshow(L);

K_iter=int32(str2double(get(handles.LRRAGR_UI_kmeans_iter,'String')));%K_means迭代次数

disp('执行谱聚类...');
idx=spe_clust(L,K_iter,LRRAGR_class_num);%谱聚类
...
if (isempty(labal_path))
    ...
    LRRAGR_result = ClusteringMeasure(gnd,idx)%指标
...
new_axes=copyobj(handles.LRRAGR_UI_rep,new_f_handle); %axes1是GUI界面内要保存图线的Tag，将其copy到不可见的figure中
